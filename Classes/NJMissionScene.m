...
- (void) setupInfoLayer {
	...
	CCLabel *name = [CCLabel labelWithString:@"Tokumaru" fontName:@"Courier-Bold" fontSize:24];
	...
	level = [CCLabel labelWithString:@"Lv.01" fontName:@"Courier-Bold" fontSize:24];
	level.position = ccp(200 + level.contentSize.width / 2, infoHeight - level.contentSize.height / 2);
	level.color = ccRED;
	[infoLayer addChild:level];
	
	exp = [CCLabel labelWithString:[NSString stringWithFormat:@"Exp %10d", 100] fontName:@"Courier-Bold" fontSize:24];
	exp.position = ccp(70 + exp.contentSize.width / 2, 5 + exp.contentSize.height / 2);
	exp.color = ccBLUE;
	[infoLayer addChild:exp];
	
	energy = [CCLabel labelWithString:@"Energy" fontName:@"Courier-Bold" fontSize:24];
	...
	money = [CCLabel labelWithString:@"Money" fontName:@"Courier-Bold" fontSize:24];
...
-(id) init
{
	if( (self=[super init] )) {
		...
		//
		//CCSprite *bg = [CCSprite spriteWithFile:@"babel_origin.png"];// rect:CGRectMake(0, 0, 1061, 800)];
...
- (void) handleSpaceType:(uint)type {
	switch (type) {
	case 5:
		[[CCDirector sharedDirector] pushScene:[CCRotoZoomTransition transitionWithDuration:1.0f scene:[BattleScene scene]]];
		break;
	default:
		break;
	}
